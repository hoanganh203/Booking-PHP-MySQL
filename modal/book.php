...
// function hoadon($id){
//     $sql = "SELECT `booking`.`id` as `id`,`booking`.`id_user` as `id_user`,`taikhoan`.`user` as `user`,`sanpham`.`name` as `name`,`sanpham`.`price` as `price`,`booking`.`date_book` as `date_book`,`booking`.`time_book` as `time_book`,`booking`.`time_nhan` as `time_nhan`,`booking`.`note` as `note` from `booking` , `taikhoan` , `sanpham` where `booking`.`id_user` = `taikhoan`.`id_user` and `booking`.`id_xebook` = `sanpham`.`id_xebook`";
